...
- **游戏仪表盘** — Canvas 实时渲染世界地图、Agent 卡片面板、事件监视器、生态图表、日志
- **系统主题** — 自动跟随系统深色/浅色模式，支持手动切换
- **中英双语** — 完整 i18n 覆盖，支持手动切换中文/英文
- **WebSocket 实时通信** — 地图状态、事件流、日志流
...
├── app/
│   ├── App.tsx                      # 路由：启动中 → 启动页 → 游戏仪表盘
│   ├── LaunchPage.tsx               # 世界列表 + 创建弹窗（含全部配置区段）
│   ├── GameDashboard.tsx            # 游戏仪表盘外壳（3 栏 + 底部面板）
...
├── hooks/
│   ├── useWebSocket.ts              # 游戏状态 WebSocket
